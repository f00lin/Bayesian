...
#(version 2: works for any number of data points or numbers of hypotheses)
    ...
    for i in data:
        ...
        if i == 1:
            for index, object in enumerate(hyp):
                dat.append(hyp[index][0])
            prob_string.append(dat)
        if i == 2:
            for index, object in enumerate(hyp):
                dat.append(hyp[index][1])
            prob_string.append(dat)
        if i == 3:
            for index, object in enumerate(hyp):
                dat.append(hyp[index][2])
            prob_string.append(dat)
    return prob_string
...
#test = [0.0021, 0.0048, 0.0108]  #should produce [0.1186, 0.2712, 0.6102]
